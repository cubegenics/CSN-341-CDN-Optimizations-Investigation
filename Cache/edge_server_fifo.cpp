...
vector<queue<int>> fifoQueues(CACHE_SIZE / SET_ASSOCIATIVITY, queue<int>());
...
{
    cacheHits++;
    cache[setIndex][accessedIndex].ttl = MAX_TTL; // Reset TTL on hit
}

// Function to find the oldest entry (FIFO) in a set
int findOldestEntry(int setIndex)
{
    int oldestIndex = fifoQueues[setIndex].front();
    fifoQueues[setIndex].pop();
    return oldestIndex;
}

// Function to handle cache miss and update LRU order
// Function to handle cache miss and replace the oldest entry with TTL
void handleCacheMiss(int setIndex, int address, int ttlValue)
{
    ...
    if (fifoQueues[setIndex].empty())
    {
        // All entries in the set are currently empty
        // Just add the new entry to the first slot
        int firstIndex = 0;
        cache[setIndex][firstIndex].valid = true;
        cache[setIndex][firstIndex].tag = address / CACHE_SIZE;
        cache[setIndex][firstIndex].data = stoi(to_send);
        cache[setIndex][firstIndex].ttl = ttlValue; // Set TTL value
        fifoQueues[setIndex].push(firstIndex);
    }
    else
    {
        // Find the oldest entry
        int oldestIndex = findOldestEntry(setIndex);

        // Replace the oldest entry with the new data
        cache[setIndex][oldestIndex].valid = true;
        cache[setIndex][oldestIndex].tag = address / CACHE_SIZE;
        cache[setIndex][oldestIndex].data = 0;
        cache[setIndex][oldestIndex].ttl = ttlValue; // Set TTL value

        // Add the new entry to the FIFO queue
        fifoQueues[setIndex].push(oldestIndex);
    }
}

// Function to simulate cache read operation with FIFO replacement policy and TTL
int readFromCache(int address, int ttlValue)
{
    ...
    int oldestIndex = findOldestEntry(setIndex);
    return cache[setIndex][oldestIndex].data;
}

// Function to update TTL values for cache entries
void updateTTL()
{
    for (int i = 0; i < CACHE_SIZE; i++)
    {
